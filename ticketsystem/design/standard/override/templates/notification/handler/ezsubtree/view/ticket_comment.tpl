...
{set-block scope=root variable=subject}Re: [TICKET][#{$object.main_node.parent.object.id}] {'"%name" was updated'|i18n('design/standard/notification', '', hash( '%name', $object.name|wash ))}[{ezini("SiteSettings","SiteURL")}]{/set-block}
...
{$object.data_map.comment.content.output.output_text|striptags()}
{section-else}
{set-block scope=root variable=subject}Re: [TICKET][#{$object.main_node.parent.object.id}] {'"%name" was created'|i18n('design/standard/notification', '', hash( '%name', $object.name|wash ))} [{ezini("SiteSettings","SiteURL")}]{/set-block}
...
{$object.data_map.comment.content.output.output_text|striptags()}
{/section}
http://{ezini("SiteSettings","SiteURL")}{cond( $use_url_translation, $object.main_node.parent.url_alias|ezurl(no),
                                               true(), concat( "/content/view/full/", $object.main_node.parent.main_node_id )|ezurl(no) )}
