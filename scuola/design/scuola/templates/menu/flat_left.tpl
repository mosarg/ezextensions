...
{def $left_menu_depth = $pagedata.current_menu|eq('LeftOnly')|choose( 1, 0 )}
        <h4>{"Contenuti"|i18n('scuola/leftmenu')}</h4>
        <ul class="menu-list">
        
        <li><a class="parent-menu"href={if eq( $ui_context, 'browse' )}{concat("content/browse/", $pagedata.path_array[$left_menu_depth].node_id)|ezurl}{else}{$pagedata.path_array[$left_menu_depth].url_alias|ezurl}{/if}>{$pagedata.path_array[$left_menu_depth].text} ^</a></li>
    ...
    {if $left_menu_items_count}
        ...
        {foreach $left_menu_items as $key => $item}
            ...
            {if eq( $item.class_identifier, 'link')}
                <li{if $li_class} class="{$li_class|implode(" ")}"{/if}><div class="first_level_menu"><a {if eq( $ui_context, 'browse' )}href={concat("content/browse/", $item.node_id)|ezurl}{else}href={$item.data_map.location.content|ezurl} target="_blank"{/if}{if $a_class} class="{$a_class|implode(" ")}"{/if} title="{$item.data_map.location.data_text|wash}">{$item.name|shorten(18)|wash()}</a></div>
            {else}
                <li{if $li_class} class="{$li_class|implode(" ")}"{/if}><div class="first_level_menu"><a href={if eq( $ui_context, 'browse' )}{concat("content/browse/", $item.node_id)|ezurl}{else}{$item.url_alias|ezurl}{/if}{if $a_class} class="{$a_class|implode(" ")}"{/if}>{$item.name|shorten(18)|wash()}</a></div>
            {/if}
            ...
            {if eq( $current_node_in_path_2, $item.node_id )}
                ...
                <ul class="submenu-list">
                    ...
                    {if eq( $subitem.class_identifier, 'link')}
                        <li{if $li_class} class="{$li_class|implode(" ")}"{/if}><div class="second_level_menu"><a {if eq( $ui_context, 'browse' )}href={concat("content/browse/", $subitem.node_id)|ezurl}{else}href={$subitem.data_map.location.content|ezurl} target="_blank"{/if}{if $a_class} class="{$a_class|implode(" ")}"{/if} title="{$subitem.data_map.location.data_text|wash}">{$subitem.name|shorten(18)|wash()}<span>> </span></a></div></li>
                    {else}
                        <li{if $li_class} class="{$li_class|implode(" ")}"{/if}><div class="second_level_menu"><a href={if eq( $ui_context, 'browse' )}{concat("content/browse/", $subitem.node_id)|ezurl}{else}{$subitem.url_alias|ezurl}{/if}{if $a_class} class="{$a_class|implode(" ")}"{/if}>{$subitem.name|shorten(18)|wash()}<span>> </span></a></div></li>
                    {/if}
                    {/foreach}
                </ul>
                {/if}
