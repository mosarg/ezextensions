                ...
                <tr id="embedlistsrcrow">
                    ...
                    <td colspan="2" id="embedlistsrccontainer">
                      <select name="location" id="location" title="{'Lets you specify where in eZ Publish to store the uploaded object.'|i18n('design/standard/ezoe/wai')}">
                        ...
                        {def $old_depth=0 $iterator=0}
                        {def $doc_folders_list=mediafolders('data')}
                        {foreach $doc_folders_list as $key=>$folders}
                            {set $iterator=0}
                             <optgroup label="{$key}">
                                {foreach $folders as $folder}
                                    {set $iterator=inc($iterator)}
                                    {if eq($iterator,1)}
                                        {set $old_depth=$folder.depth}
                                    {/if}
                                    {if $folder.can_create}
                                        <option value="{$folder.node_id}">{'-'|repeat($folder.depth|sub($old_depth))}{$folder.name|wash|shorten( 35 )}</option>
                                    {/if}
                                {/foreach}
                             </optgroup>
                        {/foreach}
