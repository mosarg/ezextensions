REGISTER B
<form enctype="multipart/form-data"  action={"/user2/register2/"|ezurl} method="post" name="Register">
...
{/section}
	...
	{def $gruppi=fetch(content,list,hash(parent_node_id,5, class_filter_type, "include", class_filter_array, array(3),'limitation', array() ))}
	{*
	<select name="scuola">
		{foreach $gruppi as $gruppo}
			 {if $gruppo.node_id|gt(100)}
			<option value="{$gruppo.node_id|wash}">{$gruppo.name|wash}</option>
			{/if}
		{/foreach}
	</select>
	*}
