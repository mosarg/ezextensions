...
class mPageData
{
    ...
    {
        return array( 'mpagedata', 'mpagedata_set', 'mpagedata_append' );
    }
    ...
    function modify( $tpl, $operatorName, $operatorParameters, $rootNamespace, $currentNamespace, &$operatorValue, $namedParameters )
    {
        switch ( $operatorName )
        {
            ...
            case 'mpagedata_set':
            case 'mpagedata_append':
            {
                self::setPersistentVariable( $namedParameters['key'], $namedParameters['value'], $tpl, $operatorName === 'mpagedata_append' );
            }break;
            case 'mpagedata':
            {                    
                $currentNodeId = 0;
                ...
                if ( isset( $parameters['current_node_id'] ) )
                {
                   ...
                   if ( isset( $parameters['set_current_node_path'] ) && $parameters['set_current_node_path'] )
                   {
                       ...
                       {
                           eZDebug::writeWarning( "Could not fetch 'current_node_id'", 'mPageData::getNodePath()' );
                       }
                   }
                }
    ...
    static public function setPersistentVariable( $key, $value, $tpl, $append = false )
    {
        ...
        // storing the value internally as well in case this is not a view that supports persistent_variable (mpagedata will look for it)
