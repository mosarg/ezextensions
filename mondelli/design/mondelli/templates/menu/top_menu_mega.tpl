...
{if and($current_node_id|eq(0),$current_user.login|ne('anonymous'),
        $module_result.ui_component|ne('ezmultiupload'))}
       ...
       {def $menu_content_restricted=ezini( 'DropDownMenuContentSettings', 'TopListRestricted', 'dropdownmenu.ini' )}
        ...
        {set $child_check=fetch(content,list_count,
                ...
                {if eq( $menu_item_first.class_identifier, 'link')}
                    <li class="{$style} zero_node"><img alt="teiera" src={$menu_item_first.data_map.icon.content[menublock].url|ezroot} />{$menu_item_first.name}
                {else}
                    <li class="{$style} zero_node"><img alt="teiera" src={$menu_item_first.data_map.icon.content[menublock].url|ezroot} /><a class="{concat('menu_',$i)} oldborder5 body_font root_menu roundedtop roundedbottom" href={$menu_item_first.url_alias|ezurl}>{$menu_item_first.name}
                {/if}
         ...
         {if $child_check|gt(0)}
                   <!--second level-->
                    ...
                    <li class="root_icon rounded">
                        ...
                        <span> <img  alt="megadrop_0" class=""src={$menu_item_first.data_map.menu_icon.content[gallerythumbnailsquare].url|ezroot}/> {$menu_item_first.data_map.menu_description.content|shorten(120)}</span>
                    </li>
