       ...
       {def $menu_content_restricted=ezini( 'DropDownMenuContentSettings', 'TopListRestricted', 'dropdownmenu.ini' )}
        ...
        {set $child_check=fetch(content,list_count,
                ...
                {if eq( $menu_item_first.class_identifier, 'link')}
                    <li class="zero_node">{$menu_item_first.name}
                {else}
                    <li class="zero_node"><a class="{concat('menu_',$i)} oldborder5 body_font root_menu" href={$menu_item_first.url_alias|ezurl}>{$menu_item_first.name}
                {/if}
         ...
         {if $child_check|gt(0)}
                   <!--second level-->
                    ...
                    <li class="root_icon">
                        ...
                        <span> <img  alt="megadrop_0" class=""src={$menu_item_first.data_map.icon.content[gallerythumbnailsquare].url|ezroot}/> {$menu_item_first.data_map.menu_description.content|shorten(120)}</span>
                    </li>
